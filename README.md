...
**Official command-line interface and TypeScript SDK for Nexus —
...
This repository hosts the two official npm packages for working with Nexus from the terminal or from TypeScript.
...
Every published release of `@agent-nexus/cli` and `@agent-nexus/sdk` carries an [npm provenance attestation](https://docs.npmjs.com/generating-provenance-statements) — a signed Sigstore bundle that ties the published tarball back to the exact GitHub Actions workflow run that built it. Verify yourself:
...
`agent-nexus` is the published face of two packages whose source is maintained in the private NexusGPT monorepo. Every merge to the monorepo automatically syncs `packages/cli/` and `packages/sdk/` here, and releases are published from this repo so that npm can attach provenance attestations.
