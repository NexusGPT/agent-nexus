...
hidden — there are none at all (see INTERNAL). Under them sit **638 command nodes**
and **545 invocable leaves**. Derive these yourself with `deriveCommandNodes()` and
...
stderr. **79 leaves build their own document with a bare `console.log`** rather
...
gate, which drives **538 of the 545 leaves** and parses each one's stdout, and
...
the 538 leaves the gate drives. And on every terminal path — `--help`,
...
**439 of the 545 leaves** carry a derived shape line on their `--help`, generated
...
than a gap: 79 write their own document, 17 branch to two shapes, 3 have no
...
545 leaves, **0 unclassified, 0 stale**, 59 classified `safe`.
...
`src/command-universe.ts` renders all 638 nodes, and the root program is a 639th
