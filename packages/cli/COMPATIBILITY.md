...
hidden — there are none at all (see INTERNAL). Under them sit **637 command nodes**
and **544 invocable leaves**. Derive these yourself with `deriveCommandNodes()` and
...
`upgrade` is the top-level command with aliases (`update`, `latest`, `up`); the
old spellings keep resolving. `task-eval` carried the alias `eval` from its own
rename until Prompt Lab phase 2 reassigned `nexus eval` to golden-conversation
authoring — type `task-eval` for AI-task evaluations.
...
stderr. **78 leaves build their own document with a bare `console.log`** rather
...
gate, which drives **537 of the 544 leaves** and parses each one's stdout, and
...
the 537 leaves the gate drives. And on every terminal path — `--help`,
...
**43 commands declare `--yes`**, and every one of them behaves identically:
...
All 43 declare the flag through `confirmable()` and ask through
`confirmDestructive()`, both in `src/util/confirm.ts`; the refusal lives in that
one helper, and no command parses `--yes` for itself.

**All 43 refuse, and that is DRIVEN rather than asserted from the source.**
...
`destructive-confirmation.scan.ts` derives **68** candidates by verb —
...
| ------------------------- | ----- | --------------------------------------------------------- |
| `CONFIRMS_BEFORE_ACTING`  | 43    | destroys, and confirms. The promise above covers these.   |
...
**439 of the 544 leaves** carry a derived shape line on their `--help`, generated
...
The remaining 105 carry **no** shape line, and that is the honest output rather
than a gap: 78 write their own document, 17 branch to two shapes, 3 have no
registration the scan can read, 7 reach no printer, and 0 is ambiguous. A default
...
544 leaves, **0 unclassified, 0 stale**, 59 classified `safe`.
...
`src/command-universe.ts` renders all 637 nodes, and the root program is a 638th
...
**0 leaves are on a deprecation cycle today**, out of the **544 paths** the last
...
3. Where the old form can keep working, it keeps working — an alias for a rename
   (`upgrade` kept `update`), a fallback for a moved field.
